...
const DepositModal: React.FC<DepositModalProps> = ({ isOpen, onClose, multisigAddress, multisigChainId }) => {
  ...
  const { isUpgraded } = useSmartAccountStore();
  ...
  const handleGrantPermission = () => {
    openModal("permissionRequest", {
      tokenAddress: usdcAddress || "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",
      amount: Number(form.watch("amount")) || 100,
      onPermissionGranted: () => {
        mutate({
          multisigAddress: multisigAddress!,
          amount: parseUnits(form.watch("amount") || "0", USDC_TOKEN.decimals),
          useDelegation: true,
        });
      },
    });
  ...
  const handleSubmit = form.handleSubmit((values: FormValues) => {
    ...
    if (isUpgraded) {
      ...
      openModal("confirm", {
        ...
        description:
          "Using a MetaMask Smart Account with ERC-7710 delegation enables permission-based x402 deposits. Would you like to upgrade?",
        confirmText: "Upgrade",
        cancelText: "Skip",
        onConfirm: () => {
          openModal("smartAccountUpgrade");
        },
        onCancel: () => {
          mutate({
            multisigAddress: multisigAddress!,
          ...
          <p className="px-5 text-sm text-grey-600 leading-5">
            ...
            {isUpgraded && (
              <span className="block mt-1 text-xs text-green-600">
                Using MetaMask Smart Account with ERC-7710 delegation
              </span>
            )}
          </p>
