...
export const useInitializeApp = () => {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      // Case 1: No accessToken - clear state, stay on current page
      if (!accessToken) {
        clearCurrentAccount();
        setIsLoading(false);
      ...
      try {
        ...
        } else if (appError.code === ErrorCode.NETWORK_ERROR || error.name === "TypeError") {
          // Backend not available — let the user connect wallet and proceed
          clearCurrentAccount();
        } else {
